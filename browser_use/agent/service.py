...
from browser_use.agent.Reports import HTMLLogger
...
loggers = HTMLLogger('report.html')
...
def log_response(response: AgentOutput) -> None:
	...
	loggers.log('INFO',f'{emoji} Eval: {response.current_state.evaluation_previous_goal}')
	loggers.log('INFO',f'🧠 Memory: {response.current_state.memory}')
	loggers.log('INFO',f'🎯 Next goal: {response.current_state.next_goal}')
	for i, action in enumerate(response.action):
		logger.info(f'🛠️  Action {i + 1}/{len(response.action)}: {action.model_dump_json(exclude_unset=True)}')
		loggers.log('INFO',f'🛠️  Action {i + 1}/{len(response.action)}: {action.model_dump_json(exclude_unset=True)}')
...
class Agent(Generic[Context]):
	...
	def __init__(
		...
		if 'grok' in self.model_name.lower():
			...
			loggers.log('INFO','⚠️ XAI models do not support use_vision=True yet. Setting use_vision=False for now...')
			self.settings.use_vision = False
		if 'grok' in (self.planner_model_name or '').lower():
			...
			loggers.log('INFO','⚠️ XAI models do not support use_vision=True yet. Setting use_vision_for_planner=False for now...')
			self.settings.use_vision_for_planner = False
		
		#allure.attach('pass', 'logged in sucessfully', allure.attachment_type.TEXT)
		...
		if self.settings.save_conversation_path:
			...
			loggers.log('INFO',f'Saving conversation to {self.settings.save_conversation_path}')
	...
	def _set_browser_use_version_and_source(self) -> None:
		...
		loggers.log('INFO',f'Version: {version}, Source: {source}')
	...
	async def step(self, step_info: Optional[AgentStepInfo] = None) -> None:
		...
		loggers.log('INFO',f'📍 Step {self.state.n_steps}')
		state = None
		model_output = None
		...
		try:
			...
			if step_info and step_info.is_last_step():
				...
				loggers.log('INFO','Last step finishing up')
			...
			try:
				...
				if (
					...
					loggers.log('INFO','Model returned empty action. Retrying...')
					...
					if not model_output.action or all(action.model_dump() == {} for action in model_output.action):
						...
						loggers.log('INFO','Model still returned empty after retry. Inserting safe noop action.')
			...
			if len(result) > 0 and result[-1].is_done:
				...
				loggers.log('INFO',f'📄 Result: {result[-1].extracted_content}')
	...
	async def get_next_action(self, input_messages: list[BaseMessage]) -> AgentOutput:
		...
		if self.tool_calling_method == 'raw':
			...
			loggers.log('INFO',f'Using {self.tool_calling_method} for {self.chat_model_library}')
			try:
				output = self.llm.invoke(input_messages)
				response = {'raw': output, 'parsed': None}
			except Exception as e:
				logger.error(f'Failed to invoke model: {str(e)}')
				loggers.log('INFO',f'Failed to invoke model: {str(e)}')
			...
			try:
				...
				loggers.log('INFO',f'Failed to parse model output: {output} {str(e)}')
		...
		if not parsed:
			try:
				...
				loggers.log('INFO',f'Failed to parse model output: {response["raw"].content} {str(e)}')
				raise ValueError('Could not parse response.')
	...
	def _log_agent_run(self) -> None:
		...
		loggers.log('INFO',f'🚀 Starting task: {self.task}')

		logger.debug(f'Version: {self.version}, Source: {self.source}')
		loggers.log('INFO',f'Version: {self.version}, Source: {self.source}')
		self.telemetry.capture(
			AgentRunTelemetryEvent(
				agent_id=self.state.agent_id,
		...
		try:
			...
			for step in range(max_steps):
				...
				if self.state.consecutive_failures >= self.settings.max_failures:
					...
					loggers.log('INFO',f'❌ Stopping due to {self.settings.max_failures} consecutive failures')
					break

				# Check control flags before each step
				if self.state.stopped:
					logger.info('Agent stopped')
					loggers.log('INFO','Agent stopped')
					break
				...
				loggers.log('INFO',f'❌ {error_message}')
			...
			loggers.log('INFO','Got KeyboardInterrupt during execution, returning current history')
			return self.state.history
	...
	async def multi_act(
		...
		for i, action in enumerate(actions):
			...
			try:
				...
				loggers.log('INFO',f'Executed action {i + 1} / {len(actions)}')
				...
				loggers.log('INFO',f'Action {i + 1} was cancelled due to Ctrl+C')
				if not results:
	...
	async def _validate_output(self) -> bool:
		...
		if not is_valid:
			...
			loggers.log('INFO',f'❌ Validator decision: {parsed.reason}')
	...
	async def log_completion(self) -> None:
		...
		loggers.log('INFO','✅ Task completed')
		if self.state.history.is_successful():
			logger.info('✅ Successfully')
			loggers.log('INFO','✅ Successfully')
		else:
			logger.info('❌ Unfinished')
			loggers.log('INFO','❌ Unfinished')

		total_tokens = self.state.history.total_input_tokens()
		logger.info(f'📝 Total input tokens used (approximate): {total_tokens}')
		loggers.log('INFO',f'📝 Total input tokens used (approximate): {total_tokens}')
	...
	async def rerun_history(
		...
		for i, history_item in enumerate(history.history):
			...
			loggers.log('INFO',f'Replaying step {i + 1}/{len(history.history)}: goal: {goal}')
			...
			if (
				...
				loggers.log('INFO',f'Step {i + 1}: No action to replay, skipping')
			...
			while retry_count < max_retries:
				try:
					...
					if retry_count == max_retries:
						...
						loggers.log('ERROR',error_msg)
						if not skip_failures:
							results.append(ActionResult(error=error_msg))
							raise RuntimeError(error_msg)
					else:
						logger.warning(f'Step {i + 1} failed (attempt {retry_count}/{max_retries}), retrying...')
						loggers.log('INFO',f'Step {i + 1} failed (attempt {retry_count}/{max_retries}), retrying...')
	...
	async def _update_action_indices(
		...
		if old_index != current_element.highlight_index:
			...
			loggers.log('INFO',f'Element moved in DOM, updated index from {old_index} to {current_element.highlight_index}')
	...
	def resume(self) -> None:
		...
		if self.browser:
			...
			loggers.log('INFO','🌎 Restarting/reconnecting to browser...')
			loop = asyncio.get_event_loop()
	...
	def stop(self) -> None:
		...
		loggers.log('INFO','⏹️ Agent stopping')
	...
	def _verify_llm_connection(self) -> bool:
		...
		loggers.log('WARNING',f'Verifying the {self.llm.__class__.__name__} LLM knows the capital of France...')
	...
	async def _run_planner(self) -> Optional[str]:
		...
		try:
			...
			loggers.log('ERROR', f'Failed to invoke planner: {str(e)}')
			raise LLMException(401, 'LLM API call failed') from e
		...
		try:
			...
			loggers.log('INFO',f'Planning Analysis:\n{json.dumps(plan_json, indent=4)}')
			...
			loggers.log('INFO',f'Error parsing planning analysis: {e}')
			loggers.log('INFO',f'Plan: {plan}')
	...
	async def close(self):
		...
		try:
			...
			loggers.log('ERROR', f'Error during cleanup: {e}')
